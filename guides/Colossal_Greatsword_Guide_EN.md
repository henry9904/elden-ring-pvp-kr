...
> [!TIP]
> Not sure what CR1, JR2, AoW, or other terms mean? → [📖 PvP Lexicon](../info/PvP_Lexicon_EN.md)
...
| Strong in trades | Difficult at high latency (unless using Poison infusion) |
...
| Phase | Description |
|-------|-------------|
| **Pressure** | Create a state where the opponent doesn't know when the attack is coming within CGS range |
| **Confusion** | Mix movement and offhand attacks to bait panic rolls |
| **Roll catch** | Catch the baited panic roll with a thrust or Heavy R2 |
...
|--------|-------------|-------|
| **Standing R1** | 17f | Wide hitbox. Good for hitting opponents strafing the Crouching R1. Even if dodged, prevents backstab and sets up Heavy R2 |
| **Crouching R1** | 17f | Fast and most versatile. **Core attack** |
| **Heavy R2** | — | **Critical attack.** Combos perfectly with Standing R1. Makes opponents vulnerable to roll catch every time they roll a Standing R1 |
...
> [!TIP]
> **Standing R1 + Heavy R2 is a core combo.** Every time the opponent rolls a Standing R1, they become more vulnerable to being roll caught by the Heavy R2. You can also redirect the Standing R1 away from the opponent's roll direction to prevent backstabs.
...
Repeatedly crouch and stand while running. The crouched hitbox lingers briefly after standing, allowing a thrust to come out faster.
...
| Application | Effect |
|-------------|--------|
| Light attack redirect | Jump catch / off-beat catch possible |
| Heavy attack redirect | Possible |
...
| After Crouching R1 two-handed | Handy tech → offhand attack before opponent can retaliate |
...
| **Standing R1 → Heavy R2** | If opponent rolls the Standing R1, Heavy R2 roll catches them. Creates a dangerous guessing game |
...
|-----------------|----------|
| Attacks immediately | Spin then thrust immediately |
...
| Method | Description |
|--------|-------------|
| Space then thrust counter | Default response |
...
> → **Use offhand weapon to check them.** You can also redirect Standing R1 away from the opponent's roll direction to prevent backstabs.

---

## 🤚 Offhand Weapon Usage

### High Poise Damage Weapons (Axe, Dane's Footwork)
...
| After Crouching R1 | Handy tech → offhand attack before opponent retaliates |
...
| **Dane's Footwork** | High stagger, fast, subtle motion | Damage and range lacking |
...
| **Watchdog's Colossal Sword** | Vertical swing Heavy R2 | Shorter than Guts GS but the vertical R2 is very effective against opponents who jump frequently |
...
|------|---------|-------------|
| 1 | HP Talisman | Erdtree's Favor / Shard of Alexander |
| 2 | Greatshield Talisman | Blue Feather Talisman |
| 3 | Arsenal Charm | — |
| 4 | Godfrey Icon 2 | — |
...
- **Retaliatory Crossed-Tree** (crouching and rolling attack +12%)

### Darcy's Note on Retaliatory Crossed-Tree

> [!TIP]
> **Retaliatory Crossed-Tree** is highly effective with CGS — it increases crouching and rolling attack damage by 12%. Consider using this over Spear Talisman. Use Spear Talisman only if already running Retaliatory Crossed-Tree.
...
| **Poison infusion** | Poison in 2~3 hits, then apply safe immediate attacks as HP drains. Effective even at high latency — CGS is great at trading, and poison forces the opponent to attack, which plays into CGS's strengths |

> [!TIP]
> Normally CGS struggles at high latency, but **Poison infusion makes it highly effective** even in those conditions.
...
| **Daggers** | Fast attacks and evasion neutralize CGS effectively |
...
> [!NOTE]
> CGS has a **good matchup against every heavy weapon class.** There is no matchup that is truly unwinnable — it all comes down to execution.
...
| **Keeps getting backstabbed** | Light/heavy attacks: 1 hit max / No dash attack or dash heavy at close range / No jump attacks that overshoot / Redirect Standing R1 away from roll direction |
| **Not using Heavy R2** | Standing R1 alone is incomplete — always have Heavy R2 ready as a follow-up roll catch |
...
| **Same playstyle at high latency** | Consider Poison infusion to maintain effectiveness at high latency |
...
- The Standing R1 + Heavy R2 combination creates a genuine dilemma for the opponent — rolling the R1 makes them vulnerable to Heavy R2
